...
class ShiftObjectsManager {
 public:
  ...
  void InsertNeutrinosHittingDetectorCollection(std::shared_ptr<Event> event, const std::map<std::string, float> &detectorParams,
                                               std::string variant, std::shared_ptr<std::map<std::string, int>> nNeutrinos);
