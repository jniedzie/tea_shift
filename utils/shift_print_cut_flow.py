...
def print_table(process_data):
  ...
  processes = sorted(
    process_data.keys(),
    key=lambda x: float(x.split("ctau-")[1]) if "ctau-" in x else 0
  )
  ...
  selections = sorted(
    selections
  )  # Sort selections alphabetically for consistency
  ...
  # Print header row
  header = f"Selection".ljust(col_width_sel) + "|" + "".join(
    f"{process}".ljust(col_width_proc) + "|" for process in processes
  )
  print(header)
  print("=" * len(header))
  ...
  for i in range(1, len(selections)):
    ...
    for process in processes:
      ...
      if prev_events is not None and curr_events is not None and prev_events != 0:
        ...
        row += f"{format_number(ratio)} +/- {format_number(ratio_err)}".ljust(
          col_width_proc
        ) + "|"
  ...
  for process in processes:
    ...
    if first_events is not None and last_events is not None and first_events != 0:
      ...
      if last_events == 0:
        ...
        total_efficiency_err = total_efficiency * (
          1 / last_events + 1 / first_events
        )**0.5

      # print enough decimal places to display the first digit that's not zero

      row += f"{format_number(total_efficiency)} +/- {format_number(total_efficiency_err)}".ljust(
        col_width_proc
      ) + "|"
    else:
      row += "N/A".ljust(col_width_proc) + "|"
...
def main():
  ...
  for process in processes:
    ...
    for key, value in hist_dict.items():
      ...
      n_scaled_err = sigma / n_initial * (
        luminosity**2 * n_raw + n_raw**2 * luminosity**2 / n_initial +
        n_raw**2 * luminosity_err**2
      )**0.5

      print(
        f"{key:30}: {n_raw:.2f} +/- {n_raw_err:.2f}\t\t{n_scaled:.2e} +/- {n_scaled_err:.2e}"
      )

      scaled_cut_flow_per_process[get_nice_name(process)
                                  ][key] = (n_scaled, n_scaled_err)
