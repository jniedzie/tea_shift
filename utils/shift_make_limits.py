...
# variants = ["shift30m"]
# variants = ["shift140m"]
# variants = ["shift30m", "shift50m", "shift80m", "shift140m", "shift160m", "shift200m", "shift250m", "shift270m", "shift280m", "shift300m"]
variants = [
  "shift50m", "shift80m", "shift140m", "shift160m", "shift200m", "shift250m",
  "shift270m", "shift280m", "shift300m"
]
    ...
    for process in processes:
      ...
      with open(config_path, "r") as config_file:
        ...
        config = config.replace(
          "signal_name = \"dummy_value\"", f"signal_name = \"{process}\""
        )
        config = config.replace(
          "variant_name = \"dummy_value\"", f"variant_name = \"{variant}\""
        )
        config = config.replace(
          "Histogram(name=\"dummy_value\"",
          f"Histogram(name=\"{histogram_name}\""
        )

        new_config_name = base_config_name.format(
          get_file_name(process, variant)
        )
...
def run_combine():
  ...
  for variant in variants:
    for process in processes:
      ...
      datacard_path = base_datacard_name.format(
        get_file_name(process, variant)
      ) + ".txt"
      combine_output_path = base_combine_output_name.format(
        get_file_name(process, variant)
      )
...
def get_limits():
  ...
  for variant in variants:
    for process in processes:
      ...
      with open(
        f"../datacards/{combine_output_path}", "r"
      ) as combine_output_file:
        combine_output = combine_output_file.read()
        r_values = [
          line.split("r < ")[1].strip() for line in combine_output.split("\n")
          if "r < " in line
        ]
