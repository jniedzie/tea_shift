...
cms_graph_path = f"../plots/limits_{histogram_name}/limits_DP_2d_cms{'_old' if doOld else ''}.root"
shift_graph_path = f"../plots/limits_{histogram_name}/limits_DP_2d_shift160{'_old' if doOld else ''}.root"
...
def main():
  ...
  for i in range(cms_graph.GetN()):
    ...
    if (x, y) in shift_points:
      ...
      print(
        f"ctau = 10^{x:.0f} m, mass = {y:.0f} GeV, CMS = {z:.1f}, SHIFT: {shift_z:.1f}, ratio: {ratio:.1f}"
      )
  ...
  print(
    f"Max ratio: {max_ratio:.1f} for ctau = 10^{max_ratio_x:.0f} m, mass = {max_ratio_y:.0f} GeV"
  )
  ...
  contour_hist.Draw(
    "cont3 same"
  )  # 'cont3' draws the contour line without filling

  ROOT.gPad.SetLogz()

  shift_graph_ratio.SetMinimum(1e-3)
  shift_graph_ratio.SetMaximum(5e2)

  shift_graph_ratio.GetXaxis().SetTitle("log_{10}(c#tau [m])")
  shift_graph_ratio.GetYaxis().SetTitle("m_{A'} [GeV]")
  shift_graph_ratio.GetZaxis().SetTitle(
    "#frac{#sigma^{SHIFT}_{benchmark}}{#sigma^{SHIFT}_{limit}} / #frac{#sigma^{CMS}_{benchmark}}{#sigma^{CMS}_{limit}}"
  )
  ...
  shift_graph_ratio.GetHistogram().GetYaxis().SetRangeUser(11, 60)

  canvas.Update()
  canvas.SaveAs(
    f"../plots/limits_{histogram_name}/limits_DP_2d_shiftOverCms{'_old' if doOld else ''}.pdf"
  )
  canvas.SaveAs(
    f"../plots/limits_{histogram_name}/limits_DP_2d_shiftOverCms{'_old' if doOld else ''}.root"
  )
