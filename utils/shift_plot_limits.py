...
histogram_name = "PredictedMuonsPair_mass"
...
if scenario == "HV":
  variants = {
    #                                                   ms  ls band color
    "cms_pythiaCollider_mZprime-15_mDH-5_mDQ-1_ctau-X":
    (-1, 2, True, ROOT.kGray + 1, f"CMS: m_{{Z'(DH)}} = 15 (5) GeV"),
    "shift160m_pythia_mZprime-15_mDH-5_mDQ-1_ctau-X":
    (-1, 1, True, ROOT.kGreen + 1, f"SHIFT: m_{{Z'(DH)}} = 15 (5) GeV"),
  ...
  variants = {
    ...
    "cms_pythiaCollider_mDarkPhoton-15_ctau-X":
    (-1, 1, True, ROOT.kGray, cms_label),
    "shift160m_pythia_mDarkPhoton-15_ctau-X":
    (-1, 1, True, ROOT.kMagenta + 1, shift_label),
  }
  ...
  variants = {
    ...
    "cms_pythiaCollider_mDarkPhoton-X_ctau-1e1":
    (-1, 1, True, ROOT.kGray, cms_label),
    "shift160m_pythia_mDarkPhoton-X_ctau-1e1":
    (-1, 1, True, ROOT.kMagenta + 1, shift_label),
  ...
  variants = {
    ...
    "_pythia_mDarkPhoton-30_ctau-1em5_distance":
    (-1, 1, True, ROOT.kViolet, "m_{DP} = 30 GeV, c#tau = 10^{-5} m"),
    # "cms_pythiaCollider_mDarkPhoton-30_ctau-1em5_distance"  : (-1, 2, False, ROOT.kViolet, ""),
    "_pythia_mDarkPhoton-30_ctau-1e1_distance":
    (-1, 1, True, ROOT.kGreen + 1, "m_{DP} = 30 GeV, c#tau = 10 m"),
  ...
  fatal(
    f"Unsupported scenario: {scenario} and variable: {variable} combination."
  )
  exit()


def get_graph_set(
  values, colors, show_band, marker_style=-1, line_style=-1, lumi_scale=1.0
):
  ...
  for i, (name, x_value) in enumerate(values):
    ...
    if marker_style == -1:
      ...
      exp_graph_1sigma.SetPointError(
        i, 0, 0, (limits[3] - limits[2]) * scale,
        (limits[4] - limits[3]) * scale
      )

      exp_graph_2sigma.SetPoint(i, x_value, central_value)
      exp_graph_2sigma.SetPointError(
        i, 0, 0, (limits[3] - limits[1]) * scale,
        (limits[5] - limits[3]) * scale
      )
...
def get_2d_graph(values, lumi_scale=1.0, colliderMode=False):
  ...
  hist = ROOT.TH2D(
    "hist", "",
    len(log_bins_10) - 1, np.array(log_bins_10), 7, 10, 80
  )
...
def create_shaded_colors(base_color, num_shades=3, min_scale=0.3):
  ...
  scales = [
    min_scale + i * (1 - min_scale) / (num_shades - 1)
    ...
    with open(path, "r") as limits_file:
      for line in limits_file:
        ...
        for part in parts:
          ...
          x_value = float(
            part.replace(f"{var}-", "").replace(f"{var}", "").replace(
              "em", "e-"
            ).replace("p", ".").replace("m", "")
          )
          break
    ...
    with open(path, "r") as limits_file:
      for line in limits_file:
        ...
        for part in parts:
          ...
          if x_var in part:
            x_value = float(
              part.replace(f"{x_var}-", "").replace("em",
                                                    "e-").replace("p", ".")
            )

          if y_var in part:
            y_value = float(
              part.replace(f"{y_var}-", "").replace("em",
                                                    "e-").replace("p", ".")
            )
...
def get_mass_from_name(name, variable_name="mDarkPhoton"):
  ...
  for part in parts:
    if variable_name in part:
      return float(
        part.replace(f"{variable_name}-", "").replace("em",
                                                      "e-").replace("p", ".")
      )
  return None
...
def main():
  ...
  legend = ROOT.TLegend(
    legend_pos[0], legend_pos[1], legend_pos[2], legend_pos[3]
  )
  ...
  for variant, params in variants.items():
    ...
    if variable == "2d":
      limits = read_2d_limits(
        f"../datacards/limits_{histogram_name}_{variant}.txt"
      )
    ...
    graphs[variant] = get_graph_set(
      limits, colors, show_band, marker_style, line_style, lumi_scale
    )

    draw_graphs(graphs[variant], first, show_band)
    if title != "":
      legend.AddEntry(
        graphs[variant][1] if show_band else graphs[variant][0], title,
        "FL" if show_band else "L" if marker_style == -1 else "P"
      )

    if variable == "mDarkPhoton":
      theory_lines[variant] = get_theory_line(colliderMode)
      canvas_ratio.cd()
      ratio_graphs[variant] = get_theory_over_limit(
        graphs[variant][0], theory_lines[variant], colors[1], line_style
      )
      ...
      canvas_ratio.cd()
      ratio_graphs[variant] = get_theory_over_limit(
        graphs[variant][0], theory_lines[variant], colors[2], line_style
      )
      ratio_graphs[variant].Draw("AL" if first else "Lsame")
  ...
  canvas.SaveAs(
    f"{output_path}/limits_{scenario}_{variable}{suffix}{'_old' if doOld else ''}.pdf"
  )
  canvas_ratio.SaveAs(
    f"{output_path}/limitsRatio_{scenario}_{variable}{suffix}{'_old' if doOld else ''}.pdf"
  )

  if graph_2d:
    graph_2d.SaveAs(
      f"{output_path}/limits_{scenario}_{variable}{suffix}{'_old' if doOld else ''}.root"
    )
